bitDrawer = function(coindata){
  ...
  x.domain(d3.extent(coindata, function(d) { return parseTime(d.Date); }));
  y.domain(
    [
    0,
    d3.max(coindata, function(d) { return d.ClosePrice; })
    ]
    );
  ...
  // function for intersection markers
          d3.selectAll(".mouse-per-line")
            .attr("transform", function(d, i) {
              //for each coindatapoint get the y value for the corresponding x value of
